...
def calc_tx_fee(amount):
    ...
    try:
        store_service.parse_store_id(data.get("store_id"))
...
def multipayout():
    ...
    try:
        store_id = store_service.parse_store_id(raw_store_id)
...
def payout(to, amount, fee):
    ...
    data = request.get_json(silent=True) or {}
    try:
        store_id = store_service.parse_store_id(data.get("store_id"))
